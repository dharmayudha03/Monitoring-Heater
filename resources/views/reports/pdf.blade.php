...
<head>
    <meta charset="UTF-8">
    <title>
        {{ request('action') === 'download' ? 'Laporan_Executive_Heater_' . date('Y-m-d') : 'Laporan Executive Performansi & Keandalan Heater - PT IRC INOAC Indonesia' }}
    </title>
...
<body>
    ...
    <div id="report-document" style="width: 100%; box-sizing: border-box;">
                    ...
                    <table style="border: none; width: 100%;">
                        <tr>
                            ...
                            <td style="border: none; text-align: left; vertical-align: middle; padding-left: 12px;">
                                <div class="title">LAPORAN EXECUTIVE PERFORMA & KEANDALAN HEATER</div>
        ...
        <!-- Ringkasan Kunci (3 Box Sejajar dalam 1 Baris HTML Table - Rock Solid) -->
        <table
            ...
            <tr>
                <td class="summary-box" style="width: 33.33%;">
                    <div style="font-size: 10px; color: #64748b; font-weight: bold; text-transform: uppercase;">Total
                        Active Heater</div>
                    <div class="summary-val">{{ $totalHeaters }} Unit</div>
                </td>
                <td class="summary-box" style="width: 33.33%;">
                    <div style="font-size: 10px; color: #64748b; font-weight: bold; text-transform: uppercase;">Total
                        Replacement</div>
                    <div class="summary-val" style="color: #dc3545;">{{ $totalReplacements }} Kali</div>
                </td>
                <td class="summary-box" style="width: 33.33%;">
                    <div style="font-size: 10px; color: #64748b; font-weight: bold; text-transform: uppercase;">Log
                        Abnormal Event</div>
                    <div class="summary-val" style="color: #d97706;">{{ $warningLogsCount + $dangerLogsCount }} Event
                    </div>
                </td>
            </tr>
        </table>

        <h4 style="margin-bottom: 8px; color: #0f172a;">1. RINGKASAN PERFORMA & KESEHATAN SELURUH HEATER</h4>
        <table class="data-table">
            ...
            <tbody>
                @foreach ($heatersSummary as $index => $hs)
                    ...
                    <tr>
                        <td>{{ $index + 1 }}</td>
                        <td style="font-weight: bold;">{{ $hs->heater_code }}</td>
                        <td>{{ $hs->heater_name }}</td>
                        <td>{{ $hs->zone }}</td>
                        <td>{{ $log ? number_format($log->current, 2) . ' A' : '-' }}</td>
                        <td><span class="badge badge-{{ $badge }}">{{ $status }}</span></td>
                        <td style="font-weight: bold;">{{ $hs->total_replacements }}x</td>
                        <td style="color: #dc3545; font-weight: bold;">{{ $hs->danger_alerts_count }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>

        <h4 style="margin-bottom: 8px; color: #0f172a; margin-top: 25px;">2. RIWAYAT PENGGANTIAN HEATER TERAKHIR
            (REPLACEMENT LOGS)</h4>
        <table class="data-table">
            <thead>
                <tr>
            ...
            <tbody>
                @foreach ($recentReplacements as $idx => $rep)
                    <tr>
                        ...
                        <td>{{ $rep->reason }}</td>
                        <td style="text-align: left;">{{ $rep->notes ?: '-' }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
        ...
        <div style="margin-top: 35px; text-align: left; color: #64748b; font-size: 10px; font-weight: 500;">
            Laporan Executive Performansi & Keandalan Heater - PT IRC INOAC Indonesia
        </div>
    </div>
    ...
    <script>
        function triggerDownload() {
            ...
            const filename = "Laporan_Executive_Heater_{{ date('Y-m-d_H-i') }}.pdf";
