...
<head>
    <meta charset="UTF-8">
    <title>
        {{ request('action') === 'download' ? 'Laporan_Histori_Heater_' . date('Y-m-d') : 'Laporan Histori Monitoring Heater - PT IRC INOAC Indonesia' }}
    </title>
...
<body>
    ...
    <div class="no-print action-toolbar">
        <div>
            <strong style="font-size: 13px; color: #0f172a;"><i class="fas fa-file-pdf text-danger mr-1"></i> Preview
                Laporan PDF Histori Logs</strong>
    ...
    <div id="report-document" style="width: 100%; box-sizing: border-box;">
                    ...
                    <table style="border: none; width: 100%;">
                        <tr>
                            ...
                            <td style="border: none; text-align: left; vertical-align: middle; padding-left: 12px;">
                                <div class="title">LAPORAN HISTORI MONITORING HEATER</div>
            ...
            <tr>
                <td class="summary-box" style="width: 33.33%;">
                    <div style="font-size: 10px; color: #64748b; font-weight: bold; text-transform: uppercase;">Total
                        Data Record</div>
                    <div class="summary-val">{{ $logs->count() }} Record</div>
                </td>
                <td class="summary-box" style="width: 33.33%;">
                    <div style="font-size: 10px; color: #64748b; font-weight: bold; text-transform: uppercase;">Unit
                        Heater Terpantau</div>
                    <div class="summary-val" style="color: #0284c7;">{{ $logs->pluck('heater_id')->unique()->count() }}
                        Unit</div>
                </td>
                <td class="summary-box" style="width: 33.33%;">
                    <div style="font-size: 10px; color: #64748b; font-weight: bold; text-transform: uppercase;">Event
                        Anomali (Warning/Danger)</div>
                    <div class="summary-val" style="color: #d97706;">
                        {{ $logs->whereIn('status', ['WARNING', 'DANGER'])->count() }} Event</div>
                </td>
            </tr>
        </table>

        <h4 style="margin-bottom: 8px; color: #0f172a;">1. DATA HISTORI MONITORING SENSOR HEATER</h4>
            ...
            <tbody>
                @foreach ($logs as $idx => $log)
                    ...
                    <tr>
                        ...
                        <td style="font-weight: bold;">{{ $log->heater->heater_code ?? '-' }}</td>
                        <td>{{ $log->heater->heater_name ?? '-' }}</td>
                        <td>{{ $log->heater->zone ?? '-' }}</td>
                        <td>{{ number_format($log->current, 2) }} A</td>
        ...
        <div style="margin-top: 35px; text-align: left; color: #64748b; font-size: 10px; font-weight: 500;">
            Laporan Histori Monitoring Heater - PT IRC INOAC Indonesia
        </div>
    </div>
    ...
    <script>
        function triggerDownload() {
            ...
            const filename = "Laporan_Histori_Heater_{{ date('Y-m-d_H-i') }}.pdf";
